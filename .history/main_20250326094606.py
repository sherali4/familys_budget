...
def get_category_keyboard():
    ...
    buttons.append([InlineKeyboardButton(
        text="✏ Изменить категорию", callback_data="edit_category")])
    buttons.append([InlineKeyboardButton(
...
async def edit_category_handler(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("Введите название категории, которую хотите изменить:")
    await state.set_state(CategoryState.editing_old)
    await callback.answer()


@dp.message(CategoryState.editing_old)
async def process_old_category(message: Message, state: FSMContext):
    await state.update_data(old_category=message.text)
